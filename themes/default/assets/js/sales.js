        ...
        $.each(item.all_group_prices, function() {
          if (gp_count == 1) {
            $("<option />", {
              value: 0,
              text: 'Default Price'
            }).appendTo(opt_group_price);
            if (item.row.price_id == 0) {
              $("<option />", {
                value: this.id,
                text: this.group_name + ' (' + formatDecimal(this.price) + ' ' + this.currency_code + ') (' + this.variant_name + ')'
              }).attr("rate", this.rate).attr("datacurrency_code", this.currency_code).attr("data-unit-id", this.unit_id).appendTo(opt_group_price);
            } else {
              $("<option />", {
                value: this.id,
                text: this.group_name + ' (' + formatDecimal(this.price) + ' ' + this.currency_code + ') (' + this.variant_name + ')'
    ...
    if (item.options !== false) {
      ...
      $.each(item.options, function() {
        ...
        $("<option />", {
          value: this.id,
          text: this.name
        }).attr("rate", item.row.cost).attr("qty_unit", this.qty_unit).attr("makeup_cost_percent", item.makeup_cost_percent).appendTo(opt);
        o++;
      });
    } else {
      $('#lbpiece').text("Piece");
      ...
      $.ajax({
        ...
        success: function(data) {
          if (data.quantity_balance && data.quantity_balance > 0) {
            //$("#pquantity").val(Number(data.quantity_balance));
            $("#exp_qty").val(Number(data.quantity_balance));
            $("#request_quantity").val(data.quantity_balance);
            $("#cur_stock_qty").val(Number(data.quantity_balance));
          }
        }
      });

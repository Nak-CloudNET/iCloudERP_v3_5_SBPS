...
function loadItems() {
	...
	if (__getItem('positems')) {
		...
		if(java_applet == 1) {
            ...
            var pos_head2 	= '';
            var table 		="";
            if ($("#suspend_name").val() != 0) {
                table = 'Room | Table: <span>' + $("#suspend_name").val() + '</span>';
            }
            var pos_head3 = table + '</span><br/>Username: ' + u_username + '<br/>Customer: ' + $('#select2-chosen-1').text() + '<br>Time In: ' + formattedDate + '<br/></h5>';
            var pos_head4 = 'Oder Drink <br>' + table + '</span><br/>Username: ' + u_username + '<br/>Customer: ' + $('#select2-chosen-1').text() + '<br>Time In: ' + formattedDate + '<br/></h5>';
            var pos_head5 = 'Order Food<br>' + table + '</span><br/>Username: ' + u_username + '<br/>Customer: ' + $('#select2-chosen-1').text() + '<br>Time In: ' + formattedDate + '<br/></h5>';
        ...
        $.each(positems, function (i, e) {
			...
			if(java_applet == 1) {
				...
				if(item_type_by!="")
				{
					...
					{
                        var oprTr = '<tr style="font-weight:bold;border-bottom: 1px solid black" class="row_' + item_id + ' order_print_food " itemcode="' + item_code + '" data-item-id="' + item_id + '"><td>#' + (an - 1) + ' ' + item_name + ' (' + item_code + ') ' + (item_note ? '[' + item_note + ']' : "") + '</td><td>' + formatDecimal(item_qty) + '</td></tr>';
					    $("#order-table-food").append(oprTr);

					}else if(item_type_by=='drink' && item_print==0){
                        var oprTr = '<tr style="font-weight:bold;border-bottom: 1px solid black" class="row_' + item_id + ' order_print_drink " itemcode="' + item_code + '" data-item-id="' + item_id + '"><td>#' + (an - 1) + ' ' + item_name + ' (' + item_code + ') ' + (item_note ? '[' + item_note + ']' : "") + '</td><td>' + formatDecimal(item_qty) + '</td></tr>';

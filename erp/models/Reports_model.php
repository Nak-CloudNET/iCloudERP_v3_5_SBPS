...
ORDER BY
    ...
    {
        $this->db->select('SUM(COALESCE(grand_total, 0)) as total_amount, SUM(COALESCE(paid, 0)) as paid', FALSE)
            ->where_in('biller_id', json_decode($supplier_id));
    ...
    {
        $this->db->from('purchases')->where_in('biller_id', json_decode($supplier_id));
        return $this->db->count_all_results();
    }
	...
	public function getSaleDailies($date)
    {
        $this->db->select("
            ...
            customer, 
            sale_status, 
            (select erp_payments.discount from erp_payments where erp_payments.sale_id = erp_sales.id) as payment_discount,
            total_discount,
            grand_total, 
...
//
   ...
   public function getCustomersByID($id=null, $wh=null) {
    ...
    public function getSalesDetailDelivery($id)
    {
        $this->db
            ->select("
			deliveries.id as id, 
    ...
    public function getAllSalesDetailDelivery($id)
    {
        $this->db->select("erp_deliveries.*,erp_companies.name as driver")
            ->join("erp_companies", "erp_companies.id=erp_deliveries.delivery_by", "left")
            ->where('erp_deliveries.id', $id)
            ->order_by('erp_deliveries.id', 'DESC');
    ...
    public function getAllSalesDetailDeliveryItems($did)
    {
        $this->db->select("erp_delivery_items.product_name,
                          erp_products.code,
                          erp_delivery_items.begining_balance as t_qty,
                          erp_delivery_items.quantity_received,
                          erp_delivery_items.ending_balance as balance,
                          erp_warehouses.name as warehouse,
                          erp_product_variants.name as variants")
            ->from('erp_delivery_items')
            ->join("erp_deliveries", "erp_delivery_items.delivery_id =erp_deliveries.id", "left")
            ->join("erp_products", "erp_products.id =erp_delivery_items.product_id", "left")
            ->join("erp_product_variants", "erp_product_variants.id = erp_delivery_items.product_id", "left")
            ->join("erp_warehouses", "erp_warehouses.id =erp_delivery_items.warehouse_id", "left")
            ->where('erp_delivery_items.delivery_id', $did)
            ->order_by('erp_deliveries.id', 'DESC');

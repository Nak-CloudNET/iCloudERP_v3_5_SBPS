...
class Sales_model extends CI_Model
{
	...
	public function getProductNumber($term, $warehouse_id, $standard, $combo, $digital, $service, $user_category, $category_id, $limit = 100)
    {
		if(preg_match('/\s/', $term))
		{
			$name = explode(" ", $term);
			$first = $name[0];
            $this->db->select('products.id, code, name, type, cost,warehouses_products.quantity, warehouses_products.quantity as qoh, price, tax_rate, tax_method, cf1, product_details,subcategory_id, details,COALESCE((SELECT GROUP_CONCAT(sp.`serial_number`) 
	...
	public function getAllInvoiceItem($product_id, $sdate, $edate)
    {
        $this->db->select('sale_items.*,
                                erp_sales.customer,
                                IF(erp_product_variants.qty_unit,erp_product_variants.qty_unit,erp_sale_items.quantity) as qty_unit,
                                erp_sales.reference_no,
                                erp_users.username as name,
                                IF(erp_sale_items.option_id, erp_product_variants.name, erp_units.name) as item_unit
                                ')
                ...
                ->order_by('erp_sale_items.quantity', 'DESC');

        if ($sdate) {
            $this->db->where($this->db->dbprefix('sales').'.date BETWEEN "' . $sdate . ' 00:00:00" and "' . $edate . ' 23:59:00"');
        }
        $q = $this->db->get_where('sale_items', array('sale_items.product_id' => $product_id));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
	...
	public function returnSales($data = array(), $items = array(), $payment = array())
    {
        ...
        if ($this->db->insert('return_sales', $data)) {
            ...
            foreach ($items as $item) {
                ...
                $this->db->insert('return_items', $item);
                $return_item_id                 = $this->db->insert_id();
                $items[$i]['transaction_type'] 	= 'PRODUCT RETURN';
                $items[$i]['transaction_id'] 	= $return_item_id;
                $items[$i]['date']              = $data['date'];
            ...
            $this->site->syncQuantity(NULL, NULL, NULL, NULL, NULL, NULL, $return_id);
	...
	public function getSaleDeliveryByID($delivery_id=Null)
    {
        ...
        $this->db->join('erp_companies', 'deliveries.customer_id = erp_companies.id', 'left');
...
public function getRielCurrency(){
	...
	public function getCurrentStockQuantityByExpDate($expiry,$product_id,$warehouse_id,$option_id){
		$condition = array(
			'erp_purchase_items.product_id'    => $product_id,
			'erp_purchase_items.warehouse_id'  => $warehouse_id,
			'erp_purchase_items.expiry'        => $expiry,
            'erp_product_variants.id'          => $option_id
		);
		
		$this->db->select('erp_purchase_items.id,erp_purchase_items.product_id,erp_purchase_items.expiry,
		erp_purchase_items.warehouse_id,
		(SUM(erp_purchase_items.quantity_balance)/erp_product_variants.qty_unit) as quantity_balance');
		$this->db->from('erp_purchase_items');
        $this->db->join('erp_product_variants', 'erp_purchase_items.product_id = erp_product_variants.product_id', 'left');
		$this->db->where($condition);
	...
	public function getAllCurrentStockQuantityByDate($product_id,$warehouse_id,$option_id){
		$condition = array(
			'erp_purchase_items.product_id'  => $product_id,
			'erp_purchase_items.warehouse_id'=> $warehouse_id,
            'erp_product_variants.id'        => $option_id
		...
		$this->db->select('erp_purchase_items.id, erp_purchase_items.product_id, erp_purchase_items.expiry,
		erp_purchase_items.warehouse_id, (SUM(erp_purchase_items.quantity_balance)/erp_product_variants.qty_unit) as quantity_balance');
		$this->db->from('erp_purchase_items');
		$this->db->join('erp_product_variants', 'erp_purchase_items.product_id = erp_product_variants.product_id', 'left');
		$this->db->where($condition);
		$this->db->group_by('erp_purchase_items.expiry');

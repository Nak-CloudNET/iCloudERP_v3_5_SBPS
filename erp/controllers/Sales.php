...
class Sales extends MY_Controller
{
	...
	function modal_views($id = NULL, $sdate = NULL, $edate = NULL)
    {
        ...
        $this->data['rows'] = $this->sales_model->getAllInvoiceItem($id, $sdate, $edate);
...
AND "'.$end_date.' 23:59:00"';
    ...
    function sbps_invoice_tax($id = NULL, $view = NULL, $save_bufffer = NULL)
    {
        $this->erp->checkPermissions('add', true, 'sales');
    ...
    function return_sale($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true && $return_id = $this->sales_model->returnSale($data, $products, $payment)) {
            ...
            $this->session->set_flashdata('message', lang("return_sale_added"));
            redirect("sales/invoice_return_sbps/" . $return_id);
        } else {
...
//            $this->erp->print_arrays($data, $products, $payment);
    ...
    function getSaleOrderitems($start = NULL, $end = NULL)
    {
        ...
        $this->datatables
            ->select("sale_order.id as id, sale_order.date, sale_order.reference_no, project.company, cust.name as customer, users.username, 
					erp_so_items.qty_order, COALESCE(SUM(erp_sale_order_items.quantity_received), 0) as qty_received,
					(erp_so_items.qty_order - COALESCE(SUM(erp_sale_order_items.quantity_received), 0)) as balance,
					(IF(ISNULL(".$this->db->dbprefix("sale_order").".delivery_status), CONCAT(erp_sale_order.id, '___', 'delivery'), CONCAT(erp_sale_order.id, '___', ".$this->db->dbprefix("sale_order").".delivery_status))) as delivery_status")
            ->from('sale_order')
            ->join('sale_order_items', 'sale_order.id = sale_order_items.sale_order_id', 'LEFT')
	...
	function getSales_items($start = NULL, $end = NULL)
    {
        ...
        if ($biller_id) {
            $this->datatables
                ...
                sales.customer,
                users.username, 
                erp_sl_items.qty_order,
                IF(erp_sale_items.option_id, COALESCE(SUM( erp_sale_items.quantity_received * erp_product_variants.qty_unit), 0), erp_sale_items.quantity_received) as qty_received,
					(erp_sl_items.qty_order - IF(erp_sale_items.option_id, COALESCE(SUM( erp_sale_items.quantity_received * erp_product_variants.qty_unit), 0), erp_sale_items.quantity_received)) as balance,
...
function invoice_concrete_angkor($id=null)
{
	...
	function getPQtyByDate(){
		...
		$warehouse_id   = $this->input->post('warehouse_id');
        $option_id      = $this->input->post('option_id');
		$curQuantities  = $this->sales_model->getAllCurrentStockQuantityByDate($product_id,$warehouse_id,$option_id);
	...
	function getCurrentStockQtyByDate(){
		...
		$curStock       = $this->sales_model->getCurrentStockQuantityByExpDate($expiry, $product_id, $warehouse_id, $option_id);
